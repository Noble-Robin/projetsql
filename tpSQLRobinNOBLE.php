...
$dsn = "mysql:host=$host;dbname=$db;charset=$charset"; // Nom de la source de données
$options = [
...
// Initialisation des tables de statuts
...
// Boucle pour générer les données générales
for ($i = 0; $i < 100; $i++) {
    // Données utilisateur
...
// Fonction pour insérer les relations produit
...
// Boucle pour les données de commandes et de paniers
// Boucle pour les données de commandes et de paniers
for ($i = 0; $i < 100; $i++){
    // Données de commande
    ...
    // Ajouter plusieurs relations produit aléatoires
    for ($j = 0; $j < 5; $j++) {
        if (rand(0, 1) === 0) {
            insertProductRelation($pdo, $relation_cart_id, $relation_command_id, $product_count, $faker);
        } else {
    ...
    // Deuxième jeu de relations produit
